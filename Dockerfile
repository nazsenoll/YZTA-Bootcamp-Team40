# 1. Python taban imajı (Debian Bookworm tabanlı)
FROM python:3.11-slim

# 2. Konteyner içi çalışma dizini
WORKDIR /app

# 3. Gerekli araçları, unixODBC'yi ve Microsoft SQL Sürücüsünü (MSODBC18) kuruyoruz
RUN apt-get update && apt-get install -y --no-install-recommends \
    ...
    && curl https://microsoft.com > /etc/apt/sources.list.u/mssql-release.list \
    && apt-get update \
...
# 7. Flask uygulamasını dış dünyaya açacak şekilde (0.0.0.0) başlatıyoruz
